...
<link rel="stylesheet" type="text/css"
	href='resources/css/loginform.css'>
<link rel="stylesheet"
	href="https://code.jquery.com/ui/1.12.1/themes/smoothness/jquery-ui.css">
<script src="https://code.jquery.com/ui/1.12.1/jquery-ui.js"></script>
<script type="text/javascript"
	src="<%=request.getContextPath()%>/resources/js/member.js"></script>
				...
				<div class="content">
					<div class="signin-cont cont">
						<form id="frm-log" method="post" action="loginaction.wd">
							<input type="text" name="m_id" id="id" class="inpt"
								required="required" placeholder="ID"> <label for="email">Your
								email</label> <input type="password" name="password" id="password"
								class="inpt" required="required" placeholder="PASSWORD">
					...
					<div class="signup-cont cont">
						<form id="frm-sign" action="signupaction.wd" method="post">
							<input type="text" name="id" id="id" class="inpt"
								required="required" placeholder="ID"> <label for="id">id</label>
							<input type="password" name="password" id="password" class="inpt"
								required="required" placeholder="PASSWORD"> <label
								for="password">password</label> <input type="text" name="name"
								id="name" class="inpt" required="required" placeholder="NAME">
							<label for="name">name</label> <input type="email" name="email"
								id="email" class="inpt" required="required" placeholder="EMAIL">
							<label for="email">email</label> <select name="country"
								id="country" class="inpts">
								<c:forEach items="${Clist}" var="country">
									<option value="${country}"
										<c:if test="${country=='Republic of Korea'}">selected</c:if>>${country}</option>
								</c:forEach>
							</select> <label for="country">country</label> <input type="text"
