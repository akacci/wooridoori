                ...
                <c:if test="${empty reservation_list}">
                	...
                	<c:forEach var="list" items="${reservation_list}">
	                   <tr style="cursor: pointer;">
	                		...
	                		<td class="sub">
	                			<input type="button" class="btn btn-sm btn-info" value="수락">
	                			<input type="button" class="btn btn-sm btn-danger" value="거절">	                		
	                		</td>
	                   </tr>
                   </c:forEach>
                </tbody>                
      ...
      <div class="pagination">
		<c:if test="${totalCount > 0}">
			<c:if test="${startPage > 1}">
				&nbsp;<a href="selectRequestList.wd?pageNum=${startPage-1}">&laquo;</a>
			</c:if>
			<c:forEach var="pg" begin="${startPage}" end="${endPage}" step="1">
				<c:if test="${pg == currentPage}">
					<a href="selectRequestList..wd?pageNum=${pg}" class="active">${pg}</a>
				</c:if>
				<c:if test="${pg != currentPage}">
					<a href="selectRequestList..wd?pageNum=${pg}">${pg}</a>
				</c:if>
			</c:forEach>
			<c:if test="${endPage < totalPage}">
				&nbsp;<a href="selectRequestList..wd?pageNum=${endPage+1}">&raquo;</a>
			</c:if>
		</c:if>
	  </div>
	</body>
