...
function create_logform(log){
	...
	for(var i=0;i<optionData.length;i++){
		var selected = "";
		if(optionData[i] == 'Korea'){
			selected = "selected";
		}
		option_nation += "<option  var='"+optionData[i]+"' "+selected+" >"+optionData[i]+"</option>";
    }
    $("#frm-sign #nation").html(option_nation);
}
function getOptionData(){
	var list_nation = ["Korea"];
	$.ajax({
		url : "membernation.wd",
		dataType : "xml",
		async: false,
		...
		success: function(data){
		    ...
		    $(data).find("item").each(function(){
		    	list_nation.push($(this).find("countryEnName").text().toLowerCase().replace(/\b[a-z]/g, function(letter) {
		    	    return letter.toUpperCase();
		    	}));
		    });
		    list_nation = list_nation.sort();
		    
		    /*
		    console.log(list_nation[0].key);
		    for(i=0;i>list_nation.length;i++){
		    	option_nation += "<option var='"+list_nation[i].key+"'>"+list_nation[i].key+"</option>";
		    }
		    $("#frm-sign #nation").html(option_nation);
		    */
		}
